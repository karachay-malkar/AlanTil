-- Restore complete canonical functions after shortened manual SQL patches.
-- Existing schema and data are preserved; apply after the single-room migration.
begin;
...
-- A newly mounted client must not inherit readiness while loading vocabulary.
create or replace function public.ashyk_room_not_ready(p_room_id uuid)
returns public.ashyk_rooms
language plpgsql
security definer
set search_path=''
...
begin
  ...
  if v_room.status<>'waiting' then return v_room; end if;
  update public.ashyk_rooms
  set host_ready_at=case when v_actor=host_user_id then null else host_ready_at end,
      guest_ready_at=case when v_actor=guest_user_id then null else guest_ready_at end,
      host_seen_at=case when v_actor=host_user_id then now() else host_seen_at end,
      guest_seen_at=case when v_actor=guest_user_id then now() else guest_seen_at end,
      updated_at=now()
...
$$;
revoke all on function public.ashyk_room_not_ready(uuid) from public,anon;
grant execute on function public.ashyk_room_not_ready(uuid) to authenticated;
