-- AlanTil 16.7 — administrative account block/unblock, distinct from the
-- social (friend-level) block in public.user_blocks. Gated by the same
-- profiles.activity_access flag every other admin RPC already checks.
-- This only flags the account (visible to the admin dashboard); it does not
-- alter sign-in/session behavior, which is out of scope for this change.
...
comment on column public.profiles.account_blocked is
  'Set by an activity_access admin via admin_block_account/admin_unblock_account. Distinct from public.user_blocks (peer-to-peer social block).';

create or replace function public.admin_block_account(p_user_id uuid)
returns boolean
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if p_user_id = v_actor then
    raise exception 'cannot block own account' using errcode = '22023';
  end if;
...
create or replace function public.admin_unblock_account(p_user_id uuid)
returns boolean
language plpgsql
security definer
set search_path = ''
...
revoke all on function public.admin_block_account(uuid) from public, anon, authenticated;
revoke all on function public.admin_unblock_account(uuid) from public, anon, authenticated;
grant execute on function public.admin_block_account(uuid) to authenticated;
grant execute on function public.admin_unblock_account(uuid) to authenticated;

comment on function public.admin_block_account(uuid) is
  'Flags a profile as account_blocked. Requires profiles.activity_access for auth.uid(); rejects blocking the caller''s own account.';
comment on function public.admin_unblock_account(uuid) is
  'Clears account_blocked on a profile. Requires profiles.activity_access for auth.uid().';

-- Surface account_blocked in the existing admin activity RPCs so the client
-- can render the correct block/unblock icon state. Bodies are otherwise
-- unchanged from 20260822211130_user_activity_admin.sql.
...
begin
  ...
  select p.nickname, p.account_blocked into v_nickname, v_account_blocked
  from public.profiles p
  ...
  return jsonb_build_object(
    'user_id', p_user_id,
    'nickname', v_nickname,
    'account_blocked', coalesce(v_account_blocked, false),
