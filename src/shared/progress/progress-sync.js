import { getCurrentAuthState, subscribeToAuth } from "../auth/auth-service.js?v=16.8.0.2";
import { normalizeId } from "../domain/word-normalizer.js?v=16.8.0.2";
import { getUserSettings, replaceUserSettings } from "../settings/user-settings-store.js?v=16.8.0.2";
import {
  enqueueProgress,
  mergeProgressQueues,
  readProgressQueue,
  ...
  updateProgressEntry,
  writeProgressQueue,
} from "./progress-queue.js?v=16.8.0.2";
import { executeProgressEntry, fetchCloudProgressState } from "./progress-repository.js?v=16.8.0.2";
import { nextUnattemptedProgressEntry, shouldDiscardProgressError } from "./progress-sync-policy.js?v=16.8.0.2";
import { mergeStationProgressRows, replaceStationProgress } from "./station-progress-store.js?v=16.8.0.2";
import { replaceUserRewards } from "./reward-store.js?v=16.8.0.2";
import { replaceRouteSettings } from "./route-settings-store.js?v=16.8.0.2";
import { mergeCloudWordProgress, WORD_PROGRESS_LOCAL_KEY } from "./word-progress-store.js?v=16.8.0.2";
import { snapshotRecoveredSession } from "./session-builders.js?v=16.8.0.2";
import { readActiveSessions, removeActiveSession } from "./session-store.js?v=16.8.0.2";
import {
  getStorageScope,
...
async function applyFavoriteState(wordIds, songIds) {
  const [{ wordFavorites }, { songFavorites }] = await Promise.all([
    import("../state/word-favorites.js?v=16.8.0.2"),
    import("../state/song-favorites.js?v=16.8.0.2"),
  ]);
...
async function applyQueueEntryLocally(entry) {
  ...
  if (entry.type === "word_favorite") {
    const { wordFavorites } = await import("../state/word-favorites.js?v=16.8.0.2");
    wordFavorites.setActive(payload.word_id, payload.is_active, { queue: false });
  } else if (entry.type === "song_favorite") {
    const { songFavorites } = await import("../state/song-favorites.js?v=16.8.0.2");
    ...
    const { getUserRewards, replaceUserRewards } = await import("./reward-store.js?v=16.8.0.2");
