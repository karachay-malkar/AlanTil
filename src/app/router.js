...
import { setAnalyticsContext, trackEvent, trackPageView } from "../shared/analytics/analytics.js?v=16.8.0.2";
import { EVENTS } from "../shared/analytics/events.js?v=16.8.0.2";
import { initializeAuth } from "../shared/auth/auth-service.js?v=16.8.0.2";
import { hasActivityAccess, whenActivityAccessReady } from "../shared/admin/admin-access.js?v=16.8.0.2";
import { screenStyleDependencies } from "./screen-registry.js?v=16.8.0.2";

const DEFAULT_STORY = "understanding";
const ASSET_VERSION = "16.8.0.2";
const FEATURE_PATHS = Object.freeze({
  practice: "../features/practice/index.js",
  ashyk: "../features/ashyk/index.js",
  friends: "../features/friends/index.js",
...
function encodeSegment(value) { return encodeURIComponent(String(value || "").trim()); }
...
function featureOf(route) { if (route === "practice.ashyk") return "ashyk"; return route === "home" ? "path" : String(route || "path.home").split(".")[0]; }
...
function targetsEqual(left, right) { return left?.route === right?.route && JSON.stringify(left?.params || {}) === JSON.stringify(right?.params || {}); }
function compactParams(parameters = {}) { return Object.fromEntries(Object.entries(parameters).filter(([, value]) => value !== undefined && value !== null && value !== "")); }
function debugSearchSuffix() { const debug = new URLSearchParams(window.location.search).get("analytics_debug"); return debug === "1" ? "?analytics_debug=1" : ""; }
function safeReferrer(value) { if (!value) return ""; try { const url = new URL(value, window.location.origin); return `${url.origin}${url.pathname}`; } catch { return ""; } }
...
export function createRouter({ shell, modal, context }) {
  ...
  function drainPendingWork() {
    ...
    if (queuedNavigation) { const request = queuedNavigation; queuedNavigation = null; void show(request.target, request.options).then(request.resolve); return; }
    if (queuedRefresh) { const options = queuedRefresh; queuedRefresh = null; void refresh(options); }
  }
  ...
  function pauseScreenTimer() { if (!activeStartedAt) return; activeDuration += performance.now() - activeStartedAt; activeStartedAt = 0; }
  function resumeScreenTimer() { if (!currentScreen || !isDocumentVisible || activeStartedAt) return; activeStartedAt = performance.now(); }
  function finishScreenTimer() {
    if (!currentScreen) return;
    pauseScreenTimer();
    if (activeDuration >= 1000) trackEvent(EVENTS.SCREEN_TIME, { screen_name: currentScreen, duration_sec: Math.floor(activeDuration / 1000), page_path: screenPagePath });
    currentScreen = ""; screenPagePath = "/"; screenOpenedAt = 0; activeDuration = 0; activeStartedAt = 0;
  }
  ...
  function navigationSuffix() { return context.telegram?.getPendingUrlSuffix?.() || debugSearchSuffix(); }
  function historyState(target, index) { return { [ROUTER_STATE_KEY]: true, index, route: target.route, params: target.params }; }
  function syncBrowserHistory(target, mode = "push") {
    const path = `${buildPath(target.route, target.params)}${navigationSuffix()}`;
    if (mode === "replace") { entries[historyIndex] = target; window.history.replaceState(historyState(target, historyIndex), "", path); return; }
    historyIndex += 1; entries.splice(historyIndex); entries[historyIndex] = target; window.history.pushState(historyState(target, historyIndex), "", path);
  }
  function resolveInitialRoute() { return parsePathname(window.location.pathname); }
  ...
  function sendPageView({ initial = false, force = false } = {}) {
    const pagePath = buildPath(current.route, current.params); const pageLocation = `${window.location.origin}${pagePath}${debugSearchSuffix()}`;
    ...
    const sent = trackPageView({ page_path: pagePath, page_location: pageLocation, page_title: document.title, page_referrer: pageReferrer, screen_name: screenNameOf(current.route) });
    if (!sent) return false; lastPageReferrer = safeReferrer(pageLocation); lastTrackedLocation = pageLocation; return true;
  }
  ...
  async function show(rawTarget, { historyMode = "push", force = false, reason = "route_change", skipLeaveCheck = false, initial = false } = {}) {
    ...
    try {
      ...
      console.error("Router mount failed", error); context.root.innerHTML = `<section class="view screen"><div class="panel"><div class="errorState">${msg("common.ne_udalos_otkryt_razdel")}</div></div></section>`; syncBackControls(); return false;
    } finally { navigating = false; if (!queuedNavigation) shell.setNavigationPending?.(current.route, false); drainPendingWork(); }
  }
  async function navigate(route, params = {}, options = {}) { const target = targetWithInheritedParams(route, params); const reason = options.reason || (["home", "path.home"].includes(route) ? "home" : "route_change"); return show(target, { historyMode: options.push === false ? "replace" : "push", force: options.force === true, reason }); }
  async function replace(route, params = {}, options = {}) { const target = targetWithInheritedParams(route, params); return show(target, { historyMode: "replace", force: options.force === true, reason: options.reason || "route_change" }); }
  ...
  function fallbackBackTarget() {
    ...
    if (["learn.study", "learn.results", "learn.set"].includes(current.route)) { if (params.dictionarySlug === "favorites") return { route: "learn.catalog", params: {} }; return { route: "learn.sections", params: compactParams({ dictionarySlug: params.dictionarySlug, sectionSlug: params.sectionSlug }) }; }
  ...
  async function back(options = {}) {
    ...
    if (historyIndex > 0) { skipNextPopLeaveCheck = true; window.history.back(); return true; }
    const fallback = fallbackBackTarget(); if (!fallback) return false;
    return show(fallback, { historyMode: "replace", force: true, reason: "back", skipLeaveCheck: true });
  }
  async function reset(route = "path.home", params = { storyType: DEFAULT_STORY }) { const target = { route, params: compactParams(params) }; historyIndex = 0; entries.length = 0; return show(target, { historyMode: "replace", force: true, reason: ["home", "path.home"].includes(route) ? "home" : "route_change" }); }
  async function handlePopState(event) {
    const state = event.state;
    if (revertingPopState) { revertingPopState = false; if (state?.[ROUTER_STATE_KEY]) historyIndex = Number(state.index) || 0; return; }
    const target = state?.[ROUTER_STATE_KEY] ? { route: state.route, params: compactParams(state.params) } : parsePathname(window.location.pathname);
    const targetIndex = state?.[ROUTER_STATE_KEY] ? Number(state.index) || 0 : Math.max(0, historyIndex - 1); const oldIndex = historyIndex;
    const leaveApproved = skipNextPopLeaveCheck; skipNextPopLeaveCheck = false;
    if (!leaveApproved && !(await mayLeave(false))) { revertingPopState = true; const delta = oldIndex - targetIndex; window.history.go(delta || 1); return; }
    historyIndex = targetIndex; entries[historyIndex] = target;
    await show(target, { historyMode: "none", force: true, reason: targetIndex < oldIndex ? "back" : "route_change", skipLeaveCheck: true });
  }
  function releaseTelegramLaunchUrl() { context.telegram?.releaseLaunchUrl?.(); const path = `${buildPath(current.route, current.params)}${debugSearchSuffix()}`; window.history.replaceState(historyState(current, historyIndex), "", path); }
  function attachTelegram(webApp) {
    const nextWebApp = webApp || null; const nextBackButton = nextWebApp?.BackButton || null;
    if (telegramBackButton === nextBackButton && telegramWebApp === nextWebApp) { syncBackControls(); return nextWebApp; }
    try { telegramBackButton?.offClick?.(handleTelegramBack); } catch (error) { console.warn("Telegram BackButton unbinding failed", error); }
    telegramWebApp = nextWebApp; telegramBackButton = nextBackButton; context.telegram?.attach?.(nextWebApp);
    try { telegramBackButton?.onClick?.(handleTelegramBack); } catch (error) { console.warn("Telegram BackButton binding failed", error); }
    syncBackControls(); releaseTelegramLaunchUrl(); return nextWebApp;
  }
  function handleTelegramBack() { void back(); }
  async function start() {
    if (started) return true; await initializeAuth(); started = true; const initial = resolveInitialRoute(); current = { route: initial.route, params: compactParams(initial.params) }; historyIndex = 0; entries[0] = current;
    const canonicalPath = initial.notFound ? "/" : buildPath(current.route, current.params); const initialUrlSuffix = `${window.location.search || ""}${window.location.hash || ""}` || navigationSuffix();
  ...
  function getCurrent() { return { ...current, params: { ...current.params }, stack: entries.slice(0, historyIndex) }; }
  const api = { start, navigate, replace, refresh, back, reset, getCurrent, parsePathname, buildPath, resolveInitialRoute, syncBrowserHistory, handlePopState, mountCurrentRoute, attachTelegram, releaseLaunchUrl: releaseTelegramLaunchUrl, releaseTelegramLaunchUrl, setAnalyticsActive, canonicalize };
  shell.backButton.addEventListener("click", () => back());
  shell.root.addEventListener("click", (event) => { const routeElement = event.target.closest("[data-route]"); if (!routeElement) return; event.preventDefault(); navigate(routeElement.dataset.route); });
  shell.bottomNav.addEventListener("click", (event) => { const routeElement = event.target.closest("[data-route]"); if (!routeElement) return; event.preventDefault(); navigate(routeElement.dataset.route); });
  window.addEventListener("popstate", handlePopState);
  document.addEventListener("visibilitychange", () => { isDocumentVisible = document.visibilityState !== "hidden"; if (isDocumentVisible) resumeScreenTimer(); else pauseScreenTimer(); });
  window.addEventListener("pagehide", finishScreenTimer); window.addEventListener("beforeunload", finishScreenTimer); window.addEventListener("pageshow", () => { if (!currentScreen && started) startScreenTimer(current.route); });
