...
import { renderSectionMenu } from "../../shared/ui/list.js?v=16.8.0.1";
import { panel } from "../../shared/ui/panel.js?v=16.8.0.1";
import { buildPlaylistRoutes } from "./routes.js?v=16.8.0.1";
...
export function renderPlaylists(context, playlists, signal) {
  ...
  const items = [
    { id: "favorites", title: msg("songs.izbrannye_pesni"), favorite: true },
    ...buildPlaylistRoutes(playlists).map(({ playlist, slug }) => ({ id: slug, title: playlist.title, playlistId: playlist.id })),
  ];

  context.root.innerHTML = panel({
    title: msg("songs.pesni"),
    body: playlists.length
      ? renderSectionMenu(items, { dataName: "playlist-slug" })
      : `${renderSectionMenu(items.slice(0, 1), { dataName: "playlist-slug" })}<div class="emptyState"><div class="emptyStateTitle">${msg("songs.pesni_poka_ne_dobavleny")}</div></div>`,
  });
