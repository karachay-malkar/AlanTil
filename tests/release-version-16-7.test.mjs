...
test("16.8 release version has one shared source for Web and Mobile runtime", async () => {
  ...
  assert.equal(mobileApp.expo.version, "16.8.0");
  assert.equal(mobileApp.expo.extra.releaseVersion, "16.8.0");
  assert.equal(mobileApp.expo.extra.architecture, "shared-core-16.7.0");
  assert.equal(mobilePackage.version, "16.8.0");
});
...
test("diorama labels are structurally below their own image without absolute positioning", async () => {
  ...
  assert.match(feature, /stationLabel beginnerDioramaLabel/);
