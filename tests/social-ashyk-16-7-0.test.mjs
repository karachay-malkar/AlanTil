...
test('Service worker serves versioned application code cache-first and Router owns lazy CSS loading',()=>{
  ...
  assert.ok(sw.includes('const VERSION = "16.7.0.25";'));
...
test('Extended statistics keeps transparent headers and a small systemic search icon',()=>{
  ...
  assert.ok(router.includes('const ASSET_VERSION = "16.7.0.25";'));
  assert.ok(bootstrap.includes('router.js?v=16.7.0.25'));
  assert.ok(index.includes('const targetVersion = "16.7.0.25";'));
  assert.ok(index.includes('app.css?v=16.7.0.25'));
  assert.ok(sw.includes('const VERSION = "16.7.0.25";'));
