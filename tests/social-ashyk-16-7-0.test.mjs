...
test('Service worker serves versioned application code cache-first and Router owns lazy CSS loading',()=>{
  ...
  assert.ok(sw.includes('const VERSION = "16.7.0.15";'));
...
test('Ashyk board renders a wood fallback before async PBR textures are ready',()=>{
  ...
  assert.match(feature,/runtime\.js\?v=16\.7\.0\.15/);
});
...
test('Extended statistics keeps transparent headers and a small systemic search icon',()=>{
  ...
  assert.ok(router.includes('const ASSET_VERSION = "16.7.0.15";'));
  assert.ok(bootstrap.includes('router.js?v=16.7.0.15'));
  assert.ok(index.includes('const targetVersion = "16.7.0.15";'));
  assert.ok(index.includes('app.css?v=16.7.0.15'));
  assert.ok(sw.includes('const VERSION = "16.7.0.15";'));
