...
test('Mobile Path and shared core use roots as the default story',()=>{const source=read('mobile/platform/path-state.js');assert.match(source,/DEFAULT_NATIVE_STORY='roots'/);assert.match(source,/route_settings_v13_1/);assert.match(read('packages/alantil-core/path-config.js'),/defaultStoryType:\s*["']roots["']/);});
