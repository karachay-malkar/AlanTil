...
function updateGlobalTestInfo() {
  ...
  function nextMatchRound(){
    ...
    setMatchProgressText(`Раунд ${matchRoundIndex}`);

    // Build 10 cards (5 words + 5 translations)
    const cards = [];
    roundWords.forEach(w => cards.push({ kind:"w", id:w.id, text:w.word }));
    roundWords.forEach(w => cards.push({ kind:"t", id:w.id, text:w.trans }));
    shuffle(cards);
    ...
    matchGrid.innerHTML = cards.map((c, idx) => `
      <button class="matchCard" type="button" data-idx="${idx}" data-kind="${c.kind}" data-id="${c.id}">
        ${escapeHtml(c.text)}
      </button>
    `).join("");

    const btns = Array.from(matchGrid.querySelectorAll(".matchCard"));
    ...
    function allMatched(){
      return btns.every(b => b.classList.contains("matched"));
    }
    ...
    btns.forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        // second pick
        const firstBtn = btns[matchSelectedIdx];
        ...
        if (isCorrect){
          ...
          // Wrong: count fails for both words (if not solved yet)
          bumpFail(first.id);
          bumpFail(second.id);
