...
function updateGlobalTestInfo() {
  ...
  function startMatchGame(){
    ...
    matchPosGroups = {};
    for (const w of matchItems) {
      const pos = (w.pos || "").trim() || "unknown";
      if (!matchPosGroups[pos]) matchPosGroups[pos] = [];
      matchPosGroups[pos].push(w);
    }
    ...
    for (let r = 0; r < roundsCount; r++) {
      const allPOS = Object.keys(matchPosGroups);
      const roundPOS = randomFrom(allPOS);

      shuffle(matchPosGroups[roundPOS]);

      const roundWords = matchPosGroups[roundPOS].splice(0, 5);
