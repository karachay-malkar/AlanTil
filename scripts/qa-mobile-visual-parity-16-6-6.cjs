...
async function enterGuestPath(page,isReference){const pathLabel=page.getByText('Путь',{exact:true}).first();await clickExact(page,'Продолжить как гость');if(isReference){await page.waitForTimeout(300);if(!(await pathLabel.isVisible().catch(()=>false))){await clickExact(page,'Продолжить как гость');}}await pathLabel.waitFor({state:'visible',timeout:30000});}
async function back(page){const button=page.getByRole('button',{name:/^(Назад|Back)$/}).first();await button.waitFor({state:'visible',timeout:15000});const clear=await button.evaluate(node=>{const r=node.getBoundingClientRect(),hit=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return hit===node||node.contains(hit)});assert.ok(clear,'Back button hit target is blocked');await button.evaluate(node=>node.click());await page.waitForTimeout(220);}
async function backActiveSession(page,isReference){await back(page);if(!isReference)return;const confirm=page.locator('[data-modal-confirm="1"]');if(await confirm.isVisible().catch(()=>false)){await confirm.click();await page.waitForTimeout(250);}if(await page.getByText('Начать тест',{exact:true}).isVisible().catch(()=>false)||await page.getByText('Начать игру',{exact:true}).isVisible().catch(()=>false)){await back(page);}}
...
async function runFlow(h,isReference=false){
  ...
  await enterGuestPath(page,isReference);await page.waitForTimeout(1000);await capture('04-path-stele');
  const stele=page.getByText('Ты прошёл большой путь.',{exact:false}).first();await stele.waitFor({state:'visible',timeout:15000});const steleFontSize=await stele.evaluate(node=>Number.parseFloat(getComputedStyle(node).fontSize)||0);await page.mouse.click(1,1);await page.waitForTimeout(250);await capture('05-path');
  ...
  await clickExact(page,'Тест');await page.getByText('Начать тест',{exact:true}).waitFor({state:'visible'});await capture('07-general-test-menu');await clickExact(page,'Начать тест');await page.getByText('Ответить',{exact:true}).waitFor({state:'visible'});await capture('08-general-test-session');await backActiveSession(page,isReference);
  await clickExact(page,'Сопоставление');await page.getByText('Начать игру',{exact:true}).waitFor({state:'visible'});await capture('09-match-menu');await clickExact(page,'Начать игру');await page.waitForTimeout(250);await capture('10-match-session');await backActiveSession(page,isReference);
...
(async()=>{
  ...
  const target=await createHarness(browser,'target',targetUrl);let targetFlow=null,targetError='';try{targetFlow=await runFlow(target,false);}catch(error){targetError=String(error?.stack||error);fs.writeFileSync(path.join(output,'target-flow-error.txt'),targetError);}
