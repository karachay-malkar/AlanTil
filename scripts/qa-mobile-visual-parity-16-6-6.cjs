...
async function backActiveSession(page,isReference){await back(page);if(!isReference)return;const confirm=page.locator('[data-modal-confirm="1"]');if(await confirm.isVisible().catch(()=>false)){await confirm.click();await page.waitForTimeout(250);}}
