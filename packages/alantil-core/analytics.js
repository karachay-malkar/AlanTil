...
export function sanitizeAnalyticsParameters(parameters = {}, { appVersion = '', debugMode = false } = {}) {
  const safe = {};
  ...
  if (appVersion !== undefined) safe.app_version = appVersion;
  if (debugMode) safe.debug_mode = true;
  return safe;
}

export function mergeAnalyticsContext(current = {}, nextContext = {}, options = {}) {
  const next = { ...current, ...sanitizeAnalyticsParameters(nextContext, options) };
  delete next.app_version;
  delete next.debug_mode;
  return next;
}
