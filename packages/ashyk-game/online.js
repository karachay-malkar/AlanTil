...
export function createAshykOnlineAdapter(client){
  ...
  function openShotStream(roomId,onVisual,onError=()=>{}){
    if(!roomId)return{send:()=>false,close:()=>{}};
    let channel=client.channel(`ashyk-shot:${roomId}`,{config:{private:true,broadcast:{self:false,ack:false}}}),subscribed=false,pendingStart=null,closed=false;
    const receive=(event)=>(message)=>{if(!closed)onVisual?.(event,message?.payload||message||{});};
    const sendNow=(event,payload)=>{if(!channel||closed||!subscribed)return false;Promise.resolve(channel.send({type:'broadcast',event,payload})).catch(onError);return true;};
    channel.on('broadcast',{event:'shot-start'},receive('shot-start')).on('broadcast',{event:'shot-frame'},receive('shot-frame')).subscribe((status,error)=>{
      if(status==='SUBSCRIBED'){subscribed=true;if(pendingStart){const next=pendingStart;pendingStart=null;sendNow(next.event,next.payload);}return;}
      subscribed=false;if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED')onError(error||status);
    });
    return{
      send(event,payload){if(closed||!channel)return false;if(!subscribed){if(event==='shot-start')pendingStart={event,payload};return false;}return sendNow(event,payload);},
      close(){closed=true;pendingStart=null;subscribed=false;if(!channel)return;try{channel.unsubscribe();}catch{}try{void client.removeChannel(channel);}catch{}channel=null;}
    };
  }
  return{createFriendInvite,acceptInvite,declineInvite,cancelInvite,getRoom,getActiveRoom,markReady,pingRoom,submitAction,resolveTimeout,claimForfeit,leaveRoom,subscribeRoom,subscribeInvite,subscribeInvites,openShotStream};
}
