...
export function createAshykGameStore({engine,words=[],random=Math.random,setTimer=globalThis.setTimeout?.bind(globalThis),clearTimer=globalThis.clearTimeout?.bind(globalThis),setRepeater=globalThis.setInterval?.bind(globalThis),clearRepeater=globalThis.clearInterval?.bind(globalThis),now=()=>Date.now()}={}){
  ...
  const makeOnlineAction=(type)=>state.gameMode==='online'&&type?{id:actionId(),type}:null;
  ...
  function startTurn(player,actionType=null){clearComputerTimer();const actor=state.player,action=makeOnlineAction(actionType),local=state.gameMode==='local'?true:state.gameMode==='computer'?player===1:player===state.localPlayer,seconds=config().humanShotSeconds;patch({player,phase:'first-shot',question:null,questionLocked:false,wrongAnswerId:null,remoteQuestionSelectedId:null,remoteQuestionSubmitting:false,remoteShotMode:null,shotSeconds:local?seconds:0,questionSeconds:0,selectedId:null,selectedFace:null,statusCode:state.gameMode==='computer'&&player===2?'statusComputer':local?'statusChoosePiece':'statusOpponent',lastOutcome:bindOutcome(action,actor),onlineAction:action});engine?.clearSelection?.();scheduleComputer();}
  function finish(finalScores,forcedWinner=null,actionType=null){clearComputerTimer();const actor=state.player,action=makeOnlineAction(actionType),winner=forcedWinner??(finalScores[0]===finalScores[1]?'draw':finalScores[0]>finalScores[1]?1:2);patch({scores:finalScores,status:'finished',winner,winByKyt:forcedWinner!==null,phase:'first-shot',question:null,questionLocked:true,shotSeconds:0,questionSeconds:0,statusCode:'statusFinished',lastOutcome:bindOutcome(action,actor),onlineAction:action});}
  function onShotSettled(result){
    ...
    if(!result.success){
      ...
      startTurn(next,actionType);return;
    }
    const remaining=result.remainingPieces??Math.max(1,state.remainingAshyks-1);
    if(result.attackerFace==='КЪЫТ'&&result.targetFace==='КЪЫТ'){showOutcome('capture',{face:'КЪЫТ',delta:0,instant:true});finish([...state.scores],current,actionType);return;}
