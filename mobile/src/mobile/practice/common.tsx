...
export function Segment<T extends string | number>({ values, value, onChange }: { values: readonly T[]; value: T; onChange: (value: T) => void }) {
  return (
    <View style={styles.segment}>
      {values.map((entry) => {
        const active = entry === value;
        return (
          <Pressable key={String(entry)} onPress={() => onChange(entry)} style={[styles.segmentItem, active && styles.segmentItemActive]}>
...
export function PrimaryButton({ title, disabled = false, onPress }: { title: string; disabled?: boolean; onPress: () => void }) {
  return (
        ...
        return (
          <View key={dictionary.id} style={styles.scopeBlock}>
            ...
            {dictionary.sections.map((section) => {
              ...
              return (
                <Pressable key={section.id} onPress={() => onToggle(key, !active)} style={styles.scopeSection}>
                  <Check active={active} />
...
export function PracticeScreen({ header, children, footer }: { header: ReactNode; children: ReactNode; footer?: ReactNode }) {
  const insets = useSafeAreaInsets();
  return (
...
const styles = StyleSheet.create({
  ...
  backButton: { width: 36, height: 36, borderRadius: 18, marginBottom: 3, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(54,50,43,0.10)', backgroundColor: 'rgba(246,242,233,0.28)' },
  backGlyph: { color: theme.colors.textMuted, fontSize: 29, lineHeight: 31, fontWeight: '300', marginTop: -2 },
  headerCopy: { position: 'absolute', left: 52, right: 52, bottom: 0, height: 42, alignItems: 'center', justifyContent: 'center' },
  ...
  segment: { minHeight: 40, padding: 3, borderRadius: 13, flexDirection: 'row', backgroundColor: theme.colors.surface2, borderWidth: 1, borderColor: theme.colors.lineSoft },
  segmentItem: { flex: 1, minHeight: 32, alignItems: 'center', justifyContent: 'center', borderRadius: 10 },
  ...
  scopeSection: { minHeight: 40, flexDirection: 'row', alignItems: 'center', gap: 10, paddingLeft: 20 },
