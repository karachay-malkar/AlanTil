...
export async function loadNativeSettings(){
  ...
  try{const stored=await AsyncStorage.getItem(KEYS.settings);hasStoredSettings=stored!==null;raw=stored?JSON.parse(stored):{};}catch{raw={};}
  const migrated=migrateStoredUserSettings(raw,hasStoredSettings);
  const normalized=normalizeUserSettings(migrated||{});
...
export async function hasCompletedNativeOnboarding(){return (await AsyncStorage.getItem(KEYS.legacyOnboarding))==='1';}
export async function markNativeOnboardingComplete(){await AsyncStorage.setItem(KEYS.legacyOnboarding,'1');}
