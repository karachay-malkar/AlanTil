...
export async function loadNativeProfile(userId){
  ...
  try{response=await nativeAuthFetch(`/rest/v1/profiles?user_id=eq.${encodeURIComponent(userId)}&select=user_id,nickname,avatar_gender,created_at,updated_at&limit=1`,{headers:{Accept:'application/json'}});}catch(error){throw new NativeProfileApiError(error?.message||'Profile load failed',{operation:'get_profile',unavailable:true});}
...
export async function createNativeProfile(userId,value){const validation=validateNicknameRule(value);if(!validation.valid||!userId)throw new NativeProfileApiError('Invalid profile input',{operation:'create_profile',code:validation.reason||'unauthorized'});let response;try{response=await nativeAuthFetch('/rest/v1/profiles?select=user_id,nickname,avatar_gender,created_at,updated_at',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({user_id:userId,nickname:validation.nickname})});}catch(error){throw new NativeProfileApiError(error?.message||'Profile create failed',{operation:'create_profile',unavailable:true});}const data=await jsonOrNull(response);if(!response.ok)throw apiError(response,data,'create_profile');const profile=firstRow(data);await writeProfileCache(userId,profile);return profile;}
export async function setNativeAvatarGender(userId,value){const gender=normalizeAvatarGender(value);if(!userId||!gender)throw new NativeProfileApiError('Invalid avatar selection',{operation:'set_avatar_gender',code:'invalid'});let response;try{response=await nativeAuthFetch(`/rest/v1/profiles?user_id=eq.${encodeURIComponent(userId)}&avatar_gender=is.null&select=user_id,nickname,avatar_gender,created_at,updated_at`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({avatar_gender:gender})});}catch(error){throw new NativeProfileApiError(error?.message||'Avatar update failed',{operation:'set_avatar_gender',unavailable:true});}const data=await jsonOrNull(response);if(!response.ok)throw apiError(response,data,'set_avatar_gender');const profile=firstRow(data)||await loadNativeProfile(userId);if(!profile)throw new NativeProfileApiError('Profile not found',{operation:'set_avatar_gender',code:'not_found'});await writeProfileCache(userId,profile);return profile;}
