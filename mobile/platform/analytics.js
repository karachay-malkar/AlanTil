...
async function analyticsEnabled(){if(enabledCache!==null)return enabledCache;enabledCache=(await loadNativeAnalyticsPreference().catch(()=>null))===true;return enabledCache;}
...
export async function setNativeAnalyticsRuntimeEnabled(enabled){enabledCache=Boolean(enabled);if(enabledCache)return true;currentScreen=null;sessionId='';try{await Promise.all([AsyncStorage.removeItem(queueKey()),AsyncStorage.removeItem(VISITOR_KEY)]);}catch{}return false;}
