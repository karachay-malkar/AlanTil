  ...
  return (
    <View style={styles.header} pointerEvents="box-none">
      <View style={styles.headerLeading} pointerEvents="box-none">
        {onBack ? <HeaderCircleButton icon={<BackIcon size={20} color={C.text2} />} onPress={onBack} accessibilityLabel="Назад" /> : null}
      </View>
...
export function Button({ children, onPress, primary = false, text = false, compact = false, disabled = false, style }) {
  return (
    <Pressable
      ...
      disabled={disabled}
      onPress={onPress}
      style={({ pressed }) => [styles.button,primary && styles.buttonPrimary,text && styles.buttonTextOnly,compact && styles.buttonCompact,disabled && styles.disabled,pressed && !disabled && styles.buttonPressed,style]}
    >
...
export function FavoriteButton({ active, onPress }) {
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={active ? 'Убрать из избранного' : 'Добавить в избранное'} onPress={onPress} style={({ pressed }) => [styles.favoriteButton, pressed && { opacity: 0.65 }]}> 
      <FavoriteIcon size={23} color={active ? C.favorite : C.text3} filled={active} />
    </Pressable>
  );
}
      ...
      {items.map(([id,label,Icon]) => {
        ...
        return (
          <Pressable key={id} onPress={() => onChange(id)} style={({ pressed }) => [styles.navItem, pressed && { opacity: 0.78 }]}>
            <View style={[styles.navBubble, active && styles.navBubbleActive]}><Icon size={id === 'path' ? 23 : 20} color={color} /></View>
...
export function Screen({ children, bottomNav = false }) {
  return <View style={[styles.screen, bottomNav && { paddingBottom: theme.control.nav }]}>{children}</View>;
}
...
const styles = StyleSheet.create({
  ...
  content:{flex:1},
  header:{position:'absolute',zIndex:30,top:0,left:0,right:0,height:theme.control.header,paddingHorizontal:10,backgroundColor:'rgba(238,233,223,0.78)'},
  ...
  buttonLabel:{fontSize:14,fontWeight:'700',lineHeight:17,color:C.text1,textAlign:'center'},
  buttonPrimaryLabel:{color:C.inverse},
  buttonTextLabel:{color:C.text2},
  buttonCompactLabel:{fontSize:10,fontWeight:'700'},
  ...
  menuTitle:{fontSize:15,fontWeight:'700',color:C.text1},
  ...
  bottomNav:{position:'absolute',zIndex:35,left:0,right:0,bottom:0,height:theme.control.nav,paddingHorizontal:12,paddingTop:2,flexDirection:'row',backgroundColor:'rgba(238,233,223,0.90)'},
  ...
  navLabel:{fontSize:10,fontWeight:'600',lineHeight:11,color:C.text3,marginTop:2},
  ...
  sectionLabelText:{fontSize:11,fontWeight:'600',letterSpacing:0.35,color:C.text2},
