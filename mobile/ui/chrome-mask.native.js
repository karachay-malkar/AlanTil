...
function FadeOverlay({edge}){const stops=gradientStops(edge);return <Svg pointerEvents="none" width="100%" height="100%" preserveAspectRatio="none" style={StyleSheet.absoluteFill}><Defs><LinearGradient id="chromeFade" x1="0" y1="0" x2="0" y2="1">{stops.map((stop,index)=><Stop key={index} offset={`${stop.offset*100}%`} stopColor={C.appBg} stopOpacity={stop.alpha*CH.nativeFadeOpacity}/>)}</LinearGradient></Defs><Rect width="100%" height="100%" fill="url(#chromeFade)"/></Svg>}
export function ChromeMask({edge,height}){const bottom=edge==='bottom';return <View pointerEvents="none" style={[styles.mask,bottom?styles.bottom:styles.top,{height}]}><MaskedView pointerEvents="none" style={StyleSheet.absoluteFill} maskElement={<MaskGradient edge={edge}/>}><BlurView pointerEvents="none" intensity={CH.nativeBlurIntensity} tint="light" experimentalBlurMethod="dimezisBlurView" style={StyleSheet.absoluteFill}/><View pointerEvents="none" style={[StyleSheet.absoluteFill,{backgroundColor:C.maskGlass}]}/></MaskedView><FadeOverlay edge={edge}/></View>}
