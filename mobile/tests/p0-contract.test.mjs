...
// 15.0.2 release gate: this contract intentionally travels with the final unified source.
...
test('release version is aligned across Expo, npm and lockfile', async () => {
  ...
  assert.equal(app.expo.version, '15.0.2');
  assert.equal(app.expo.extra.releaseVersion, '15.0.2');
  assert.equal(app.expo.extra.visualParity, '15.0.2-unified-core-web-parity-4options');
  assert.equal(app.expo.android.versionCode, 10);
  assert.equal(app.expo.ios.buildNumber, '10');
  assert.equal(pkg.version, '15.0.2');
  assert.equal(lock.version, '15.0.2');
  assert.equal(lock.packages[''].version, '15.0.2');
