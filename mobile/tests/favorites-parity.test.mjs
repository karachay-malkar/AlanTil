...
test('favorites pass the selected words and direction to the shared learning engine', async () => {
  ...
  assert.match(engine, /filterLearnWordsBySelection\(words, params\.selectedIds\)/);
  assert.match(engine, /normalizeLearnDirection\(params\.direction\)/);
  assert.match(engine, /createLearnState/);
