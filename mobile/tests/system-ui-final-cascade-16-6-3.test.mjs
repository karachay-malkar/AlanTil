...
test('Mobile system UI consumes final Web cascade instead of the early theme default',()=>{
  ...
  assert.match(visual,/control:Object\.freeze\([^\n]*header:42/);
  assert.match(webShell,/appHeaderAction[\s\S]*width:36px;[\s\S]*backdrop-filter:blur\(8px\)/);
  assert.match(webChrome,/\.btn\{border-radius:14px;box-shadow:0 3px 12px/);
});
...
test('Web controls use one platform-aware glass layer and never nest Expo BlurView in chrome buttons',()=>{
  ...
  assert.match(components,/function NativeChromeMask/);
...
test('Segmented controls expose the final per-feature active surfaces',()=>{
  assert.match(visual,/settingsActiveAlpha:\.72/);
  assert.match(visual,/setActiveAlpha:\.82/);
  assert.match(visual,/testActiveAlpha:\.86/);
  assert.match(visual,/songsActiveAlpha:\.84/);
