...
test('Path scroll no longer re-renders the whole route on every scroll event',()=>{
  ...
  assert.match(pathScreen,/const onPathScroll=\(event\)=>\{const offset=event\.nativeEvent\.contentOffset\.y;offsetRef\.current=offset;routeScaleRef\.current\?\.updateOffset\(offset\);if\(positionedRef\.current\)stationWindow\.update\(offset,viewportHeightRef\.current,activeStory\);\}/);
