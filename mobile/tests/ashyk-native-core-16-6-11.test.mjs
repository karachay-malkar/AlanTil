...
test('Mobile Ashyk bonus question uses the shared Test answer control and selected state',()=>{const screen=read('mobile/screens/ashyk.js');assert.match(screen,/OptionChoice/);assert.match(screen,/selected=\{selected\}/);assert.match(screen,/setSelectedId/);assert.match(screen,/role="test\.submit"/);assert.match(screen,/questionOverlay/);});
