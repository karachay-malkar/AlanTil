...
test('current release metadata remains coherent after 16.6.2 baseline',()=>{
  assert.equal(app.expo.version,'16.6.3');
  assert.equal(pkg.version,'16.6.3');
  assert.equal(app.expo.extra.releaseVersion,'16.6.3');
  assert.equal(app.expo.android.versionCode,26);
  assert.equal(app.expo.ios.buildNumber,'26');
  assert.match(settingsChild,/>16\.6\.3</);
});

test('canonical header token follows Web theme rather than Profile root override',()=>{
...
test('shared chrome buttons and bottom navigation follow final Web shell',()=>{
...
test('shared segmented control uses Web 28px item geometry',()=>{
...
test('Practice menu uses Web 68px flat rows and exact type geometry',()=>{
...
test('Favorites uses Web set-preparation offsets and flat word rows',()=>{
  ...
  assert.match(favorites,/list:\{[^}]*paddingTop:theme\.control\.header\+54[^}]*paddingHorizontal:14[^}]*paddingBottom:\d+/s);
...
test('Story Word List moves animated search into header and matches 52px Web rows',()=>{
  ...
  assert.match(storyWords,/Animated\.timing\(searchProgress/);
  assert.match(storyWords,/OverflowMarquee/);
...
test('Onboarding remains a max-560 Web-derived setup flow with explicit progress',()=>{
  assert.match(webOnboarding,/learningSetupPane\{width:min\(100%,560px\)/);
  assert.match(onboarding,/maxWidth:560/);
  assert.match(onboarding,/progressCell/);
  assert.match(onboarding,/CompactSegmentedControl/);
  assert.match(onboarding,/persist\('account'\)/);
  assert.match(onboarding,/persist\('guest'\)/);
  assert.doesNotMatch(onboarding,/fullButton:\s*\{[^}]*minHeight:\s*44/s);
});

test('Account uses canonical avatar assets and Web flat panel geometry',()=>{
...
test('Profile root replaces brand header with Web primary tabs, guest/setup states and avatar frame',()=>{
  ...
  assert.match(profile,/avatarFrameLocked/);
  assert.match(profile,/if\(incomplete\)/);
...
test('Path geometry keeps final Web chrome dimensions and adds Story Stele/interactive scale',()=>{
  ...
  assert.match(pathScreen,/routeScale:\{[^}]*right:(?:theme\.path\.scaleRight|12)[^}]*top:'20%'[^}]*bottom:'20%'[^}]*width:theme\.path\.scaleWidth/s);
  assert.match(pathScreen,/story-stele\.webp/);
  assert.match(pathScreen,/jumpScale/);
});

test('does not reintroduce old Path 58px/66px geometry',()=>{
...
test('Station uses final Web chrome offsets and flat statistics layout',()=>{
  ...
  assert.match(station,/styles\.attemptsFlat/);
  assert.match(station,/attemptPassed/);
  assert.match(station,/attemptFailed/);
});