...
export function ProfileGate({ words, settings, onSettingsChange, onAccount, onGuest, onOpenStory }) {
  ...
  const resolveProfile = useCallback(async (nextSession) => {
    if (!nextSession?.user?.id) { setProfile(null); setChecking(false); return null; }
    setChecking(true);
    try { const next = await loadNativeProfile(nextSession.user.id); setProfile(next); return next; }
    catch { setProfile(undefined); return undefined; }
    finally { setChecking(false); }
  }, []);

  useEffect(() => {
    let alive = true;
    const apply = (nextSession) => { if (!alive) return; const value = nextSession || null; setSession(value); setAuthReady(true); void resolveProfile(value); };
  ...
  return <ProfileMainArea words={words} settings={settings} onSettingsChange={onSettingsChange} onAccount={() => onAccount?.('open')} onOpenStory={onOpenStory} />;
}
