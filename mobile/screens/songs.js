...
function SongCatalog({ songs, onOpen, favoriteIds, onFavorite }) {
...
export function SongsScreen({ songs = [], words = [], onBack, favoriteIds = new Set(), onFavorite }) {
  ...
  return <SongCatalog songs={songs} onOpen={setActiveSong} favoriteIds={favoriteIds} onFavorite={onFavorite} onBack={onBack} />;
}

const styles = StyleSheet.create({
  catalogScroll:{paddingTop:theme.control.header+12,paddingHorizontal:12,paddingBottom:24},
  searchBar:{gap:6,marginBottom:10},
  searchInput:{height:32,minHeight:32,borderWidth:1,borderColor:C.line,borderRadius:999,backgroundColor:'rgba(255,255,255,.28)',paddingHorizontal:10,fontSize:12,color:C.text1},
  modeList:{alignSelf:'flex-end',flexDirection:'row',padding:2,borderWidth:1,borderColor:C.line,borderRadius:999},
  modeItem:{minHeight:26,paddingHorizontal:7,borderRadius:999,alignItems:'center',justifyContent:'center'},
  modeItemActive:{backgroundColor:'rgba(246,242,233,.84)'},
  modeText:{fontSize:9,fontWeight:'700',color:C.text3},
  ...
  timelineWrap:{flex:1},
  timeline:{height:4,borderRadius:999,backgroundColor:C.line,overflow:'hidden'},
  timelineFill:{width:'0%',height:4,backgroundColor:C.accent},
  timeRow:{flexDirection:'row',justifyContent:'space-between',marginTop:5},
  time:{fontSize:10,color:C.text2},
  lyrics:{gap:18,paddingBottom:20},
  stanza:{paddingVertical:10,paddingLeft:10,borderLeftWidth:2,borderLeftColor:C.line},
