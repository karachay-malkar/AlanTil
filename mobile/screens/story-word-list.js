...
export function StoryWordListScreen({story,settings,favorites,setFavorites,onBack}){
  ...
  const [query,setQuery]=useState(''),[searchOpen,setSearchOpen]=useState(false);const searchProgress=useRef(new Animated.Value(0)).current;
  ...
  const animatedWidth=searchProgress.interpolate({inputRange:[0,1],outputRange:[36,searchWidth]});
  const closeSearch=()=>{setQuery('');setSearchOpen(false);};
  const trailing=<Animated.View style={[styles.headerSearchWrap,{width:animatedWidth}]}>{searchOpen?<View style={styles.headerSearch}><TextInput autoFocus value={query} onChangeText={setQuery} autoCorrect={false} autoCapitalize="none" returnKeyType="search" placeholder={searchLabel} placeholderTextColor={C.text3} accessibilityLabel={searchLabel} style={styles.headerSearchInput}/><Pressable accessibilityRole="button" accessibilityLabel={searchLabel} onPress={closeSearch} style={styles.searchClose}><Text style={styles.searchCloseText}>×</Text></Pressable></View>:<HeaderCircleButton icon={<SearchIcon size={20} color={C.text2}/>} accessibilityLabel={searchLabel} onPress={()=>setSearchOpen(true)}/>}</Animated.View>;
  return <Screen><Topography opacity={0.20}/><Header title={searchOpen?'':title} onBack={onBack} trailing={trailing}/><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>{visible.length?visible.map(({catalog,sections})=><View key={String(catalog?.dictionaryId||catalog?.catalogId||catalog?.name)}>{catalog?.name?<Text style={styles.catalogTitle}>{catalog.name}</Text>:null}{sections.map(({section,entries})=><View key={String(section?.sectionId||section?.id||section?.name)}>{isThematicCatalog(catalog)&&section?.name?<Text style={styles.sectionTitle}>{section.name}</Text>:null}{entries.map((entry)=><View key={String(entry.word.id)} style={styles.row}><Text style={styles.ordinal}>{entry.ordinal}.</Text><View style={styles.copy}><OverflowMarquee textStyle={styles.word}>{entry.word.word}</OverflowMarquee><OverflowMarquee textStyle={styles.trans}>{entry.word.trans}</OverflowMarquee></View><FavoriteButton active={favorites.has(String(entry.word.id))} onPress={()=>setFavorites(toggleFavorite(favorites,entry.word.id).ids)}/></View>)}</View>)}</View>):<View style={styles.empty}><Text style={styles.emptyText}>—</Text></View>}</ScrollView></Screen>;
}

const styles=StyleSheet.create({
  headerSearchWrap:{height:36,overflow:'visible'},headerSearch:{height:36,flexDirection:'row',alignItems:'center',justifyContent:'flex-end',gap:6},
  ...
  copy:{flex:1,minWidth:0,height:44,justifyContent:'center',borderBottomWidth:1,borderBottomColor:'rgba(54,50,43,.0864)',overflow:'hidden'},
