...
export function StationTestScreen({ station, allWords, mode='kb', onBack }) {
  ...
  useEffect(()=>{let alive=true;(async()=>{const interrupted=await loadNativeSessionSnapshot('station-test');const next=buildStationTestSessionState({station,optionWords:allWords,mode,interrupted,id:`station-${Date.now()}`,startedAt:new Date().toISOString()});if(alive)setSession(next);})();return()=>{alive=false;};},[station,allWords,mode]);
  ...
  if(!session)return <Screen><Header title="Тест этапа" onBack={onBack}/><View style={styles.loading}><Text style={styles.loadingText}>Восстанавливаем попытку…</Text></View></Screen>;
  ...
  const answer=(optionId)=>{applyStationTestAnswer(session,optionId);saveNativeSessionSnapshot('station-test',stationTestActiveSnapshot(session)).catch(()=>{});setSession(session);redraw((v)=>v+1);};
  const back=()=>{saveNativeSessionSnapshot('station-test',stationTestActiveSnapshot(session)).catch(()=>{});onBack();};
  return <Screen><Header title="Тест этапа" subtitle={`${session.index+1}/${session.questions.length}`} onBack={back}/><ScrollView contentContainerStyle={styles.testScroll}><View style={styles.progress}><ProgressBar value={progress}/></View><View style={styles.promptBlock}><Text style={styles.promptMeta}>{session.mode==='ru'?'РУССКИЙ → АЛАН':'АЛАН → РУССКИЙ'}</Text><Text style={styles.prompt}>{prompt}</Text></View><View style={styles.options}>{question.options.map((option)=><Pressable key={`${question.item.id}-${option.id}`} onPress={()=>answer(option.id)} style={({pressed})=>[styles.option,pressed&&styles.optionPressed]}><Text style={styles.optionText}>{option.text}</Text></Pressable>)}</View></ScrollView></Screen>;
}
