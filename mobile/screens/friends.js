...
function Row({rank=0,nickname,gender,rightValue,secondary,actions,rowHeight,rowType}){
  return <View style={[s.row,{height:rowHeight,minHeight:rowHeight}]}>
    {rank?<View style={s.rankWrap}><MedalOrRank rank={rank}/></View>:null}
    <View style={s.rowBody}>
      <View style={s.rowName}><GenderIcon size={14} color={genderColor(gender)}/><Text numberOfLines={1} style={[s.rowNickname,{fontSize:rowType?.primary}]}>{nickname||'—'}</Text></View>
      {secondary?<Text style={[s.rowMeta,{fontSize:rowType?.secondary}]}>{secondary}</Text>:null}
    </View>
    {rightValue?<Text style={[s.rowValue,{fontSize:rowType?.service}]}>{rightValue}</Text>:null}
...
export function FriendsScreen({settings={},userId='',onSignIn,onOpenAshyk,onOpenExtendedStats}){
  ...
  if(mode==='friends'){
    content=<>
      {snapshot.ashyk_invites.length?<Section title={t('invites')}>{snapshot.ashyk_invites.map((invite)=><Row {...rowVisual} key={invite.invite_id} nickname={invite.nickname} gender={invite.avatar_gender} actions={<View style={s.actionsRow}><IconBtn label={t('accept')} active onPress={()=>acceptInvite(invite)}><CorrectIcon size={16} color={C.successStrong}/></IconBtn><IconBtn label={t('decline')} onPress={()=>declineInvite(invite)}><WrongIcon size={16} color={C.dangerStrong}/></IconBtn></View>}/>)}</Section>:null}
      ...
      <Section title={t('friends')}>{snapshot.friends.length?snapshot.friends.map((user)=><Row {...rowVisual} key={user.user_id} nickname={user.nickname} gender={user.avatar_gender} secondary={t('streak',{count:Number(user.streak_days)||0})} actions={<View style={s.actionsRow}><IconBtn label={t('remove')} onPress={()=>void mutate(`remove:${user.user_id}`,()=>removeNativeFriend(user.user_id))}><UserMinusIcon size={16} color={C.text2}/></IconBtn><IconBtn label={t('block')} onPress={()=>void mutate(`block:${user.user_id}`,()=>blockNativeUser(user.user_id))}><BlockIcon size={16} color={C.dangerStrong}/></IconBtn></View>}/>):<Text style={s.empty}>{t('emptyFriends')}</Text>}</Section>
      {snapshot.blocked.length?<Section title={t('blocked')}>{snapshot.blocked.map((user)=><Row {...rowVisual} key={user.user_id} nickname={user.nickname} gender={user.avatar_gender} actions={<IconBtn label={t('unblock')} active onPress={()=>void mutate(`unblock:${user.user_id}`,()=>unblockNativeUser(user.user_id))}><CorrectIcon size={16} color={C.successStrong}/></IconBtn>}/>)}</Section>:null}
    </>;
  }else{
    const rows=query.trim()?searchResults:leaderboard;
    content=<Section title={t('rating')}>
      {busy&&!rows?.length?<ScreenState>{t('loading')}</ScreenState>:(rows||[]).length?(rows||[]).map((user,index)=><Row {...rowVisual} key={user.user_id} rank={query.trim()?0:(Number(user.rank)||index+1)} nickname={user.nickname} gender={user.avatar_gender} rightValue={formatRating(user.rating_score)} actions={user.user_id===userId?null:<RelationActions user={user} t={t} onAdd={onAdd} onAccept={onAccept}/>}/>):<Text style={s.empty}>{t(query.trim()?'emptySearch':'emptyFriends')}</Text>}
    </Section>;
  }

  const tabs=[['rating',t('rating')],['friends',t('friends')],...(hasStats?[['stats',t('extendedStats')]]:[])];
  return <Screen bottomNav>
    <ProfileTabs items={tabs} activeId={mode} onChange={(id)=>id==='stats'?onOpenExtendedStats?.():setMode(id)} style={s.tabs}/>
