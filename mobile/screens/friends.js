...
export function FriendsScreen({settings={},userId='',mode='rating',onModeChange,onSignIn,onOpenAshyk,onOpenStatsUser}){
  ...
  const mutate=async(key,fn)=>{if(busy)return;setBusy(key);setError('');try{await fn();await refresh();if(visibleMode==='rating')await loadRank();if(query)await runSearch(query);}catch(e){setError(e?.message||t('error'));}finally{setBusy('');}};
  ...
  const tabs=[['rating',t('rating')],['friends',t('friends')],...(accessState==='allowed'?[['stats',t('extendedStats')]]:[])];
  const changeMode=(id)=>{setSearchOpen(false);setQuery('');if(id==='stats'&&accessState!=='allowed'){onModeChange?.('rating');return;}onModeChange?.(id);};
  return <Screen bottomNav topChromeDepth={theme.chrome.screenDepths.friends.top} bottomChromeDepth={theme.chrome.screenDepths.friends.bottom}><ProfileTabs items={tabs} activeId={visibleMode} onChange={changeMode} style={s.tabs}/>{visibleMode==='stats'?<View style={s.statsBody}><AdminUsersPane settings={settings} onOpenUser={onOpenStatsUser}/></View>:<ScrollView style={s.body} contentContainerStyle={[s.scroll,{paddingBottom:bottomPadding}]} keyboardShouldPersistTaps="handled">{error?<Text style={s.error}>{error}</Text>:null}{visibleMode==='rating'?<View style={s.searchBar}><HeaderCircleButton icon={<SearchIcon size={theme.chrome.actionIconSize} color={C.text2}/>} onPress={()=>{setSearchOpen(v=>!v);if(searchOpen)setQuery('');}} accessibilityLabel={t('search')}/>{searchOpen?<TextInput value={query} onChangeText={setQuery} autoCapitalize="none" autoCorrect={false} placeholder={t('searchPlaceholder')} placeholderTextColor={C.text3} style={s.searchInput}/>:null}</View>:null}{loading&&!snapshot.friends.length&&!snapshot.incoming.length&&!snapshot.ashyk_invites.length&&visibleMode==='friends'?<ScreenState>{t('loading')}</ScreenState>:content}</ScrollView>}</Screen>;
}
const s=StyleSheet.create({
  tabs:{position:'absolute',zIndex:theme.chrome.layers.tabs,elevation:theme.chrome.layers.tabs,shadowColor:'transparent',top:theme.chrome.profileTabs.top,left:theme.chrome.profileTabs.side,right:theme.chrome.profileTabs.side,height:theme.chrome.profileTabs.height},
  body:{flex:1,minHeight:0},
  scroll:{paddingTop:theme.chrome.screenDepths.friends.top,paddingHorizontal:16,gap:14},
  statsBody:{flex:1,minHeight:0,paddingBottom:theme.control.nav+theme.chrome.contentRestGap},
  searchBar:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'transparent'},
