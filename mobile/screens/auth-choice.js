...
export function AuthChoiceScreen({settings=DEFAULT_USER_SETTINGS,onAuthenticated,onGuest}){
  ...
  return <Screen><Topography opacity={0.22}/><View style={styles.root}><View style={styles.pane}><View style={styles.copy}><Text style={[styles.title,type.title]}>{msg('account.akkaunt')}</Text><Text style={[styles.body,type.body]}>{msg('account.voyti_ili_prodolzhit_kak_gost')}</Text></View>{error?<InlineMessage type="error">{error}</InlineMessage>:null}<View style={styles.actions}><AuthProviderButton label={msg('account.voyti_cherez_google')} onPress={signIn} loading={busy} disabled={busy}/><Button action style={styles.guestButton} onPress={onGuest} disabled={busy}>{msg('account.prodolzhit_kak_gost')}</Button></View></View></View></Screen>;
}

const styles=StyleSheet.create({root:{flex:1,justifyContent:'center',paddingHorizontal:16,paddingVertical:24},pane:{width:'100%',maxWidth:520,alignSelf:'center',gap:18},copy:{gap:8,alignItems:'center'},title:{color:C.text1,textAlign:'center'},body:{color:C.text2,textAlign:'center',maxWidth:420},actions:{gap:10},guestButton:{width:'100%'}});
