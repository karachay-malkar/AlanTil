...
const MASTER_GAIN=.9;
const PROFILES=Object.freeze({ashyk:{threshold:.07,fullScale:5.8,cooldownMs:32,gain:.98},board:{threshold:.09,fullScale:7.2,cooldownMs:36,gain:.84},rim:{threshold:.12,fullScale:8.2,cooldownMs:40,gain:.88}});
...
export function createNativeAshykAudio(){
  ...
  const playSegment=(name,start,duration,volume,rate=1,minRate=.84,maxRate=1.14)=>{if(disposed)return;const slot=take(name);if(!slot)return;const player=slot.player;if(slot.timer)clearTimeout(slot.timer);try{player.pause();player.volume=clamp(volume*MASTER_GAIN,0,1);player.playbackRate=clamp(rate,minRate,maxRate);Promise.resolve(player.seekTo(Math.max(0,start))).then(()=>{if(disposed)return;player.play();slot.timer=setTimeout(()=>{try{player.pause();}catch{}slot.timer=0;},Math.max(35,duration*1000/player.playbackRate));}).catch(()=>{});}catch{}};
